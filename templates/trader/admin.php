...
switch (filter_input(INPUT_GET, 'q')) {
    case 'manage_users':
        ...
        break;
    case 'edit_user':
        $user = $admin->get_user_info(filter_input(INPUT_GET, 'id'));
        edit_user($user);
        break;
    case 'update_user':
        ...
        break;
    case 'delete_user':
        $admin->delete_user(filter_input(INPUT_GET, 'id'));
        header('Location: ' . $config[site][url] . '/?p=admin&q=manage_users');
        break;
    ...
    case 'delete_category':
        $admin->delete_category(filter_input(INPUT_GET, 'id'));
        $cat_list = $admin->list_categories();
...
function manage_users($array) {
    ...
    $count = 0;     // Total Users
    $acount = 0;    // Total Active Users
    $icount = 0;    // Total Inactive Users
    foreach ($array as $user) {
        ...
        echo '<td><a href="mailto:' . $user[email1] . '">Email User</a></td>';
        echo '<td>';
    ...
    echo '<h4>Total Active: ' . $acount . '</h4>';
    echo '<h4>Total Inactive: ' . $icount . '</h4>';
}
