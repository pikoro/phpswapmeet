...
if ($_POST[edit_item]) {
    //print_r($_POST);
    $error = $items->edit(filter_input(INPUT_POST, 'id'));
    if (is_array($error)) {
        ...
        echo '<meta http-equiv="refresh" content="3;url=http://' . $config[site][url] . '/?p=desktop" />';
    }
} else {
    ...
    <form id="edit_item" name="edit_item" enctype="multipart/form-data" method="POST" action="?p=edit_item">
        ...
        <table>
            ...
            <tr><th>Item Name</th><td><input type="text" name="name" width="50" value="<?= $item[name] ?>" /> (50 Character Limit)</td></tr>
            ...
            <tr><th>Condition</th><td><select name="condition" id="condition">
                        ...
                        if ($item[condition] == 'well worn') {
                            echo 'selected';
                        }
                        ?>>Well Worn</option>
                        <option value="falling apart" <?
                        if ($item[condition] == 'falling apart') {
                            echo 'selected';
                        }
                        ?>>Falling Apart</option>
                        <option value="broken" <?
            ...
            <tr><th>Post Type</th><td><select name="post_type" id="post_type">
                    ...
                    ?>>Sale</option>
                        <option value="swap or sell" <?
                if ($item[post_type] == 'swap or sell') {
                    echo 'selected';
                }
                ?>>Swap or Sell</option>
                        <option value="free" <?
