...
if ($_POST) {
    if ($_POST[reply]) {
        $messages->send(filter_input(INPUT_POST, 'from'), filter_input(INPUT_POST, 'to'), filter_input(INPUT_POST, 'subject'), filter_input(INPUT_POST, 'message'));
    ...
    switch (filter_input(INPUT_GET, 'q')) {
        case 'reply':
            reply(filter_input(INPUT_GET, 'mid'));
            break;
    }
    if (!$_GET[q]) {
        if (!$_GET[fid]) {
                ...
                if ($messages->check_folder_permissions($_SESSION[id], $fid) == TRUE) {
                    ...
                    for ($i = 0; $i < count($msgs); $i++) {
                        echo '<tr><td>' . ucwords($users->get_username($msgs[$i][from])) . '</td>
                                  ...
                                  <td><a';
                        if ($msgs[$i][status] == 1) {
                            echo ' style="font-weight:normal; color:grey;" ';
                        }
