  ...
  <div id="header">
    ...
    <div id="nav">
      ...
      <!-- To make the current page menu item highlighted, use the id="active" for the current page -->
        <li<? if(!$_GET[p]) { echo ' id="active"';} ?>><a href="<?= $config[site][url] ?>">Top</a></li>
<?  if($_SESSION[active] == 1){
         echo '<li ';
         if($_GET[p] == 'desktop') { echo 'id="active"';}
         echo '><a href="?p=desktop">My Desktop</a></li>';
		 }
?>
        <li<? if($_GET[p] == 'news') { echo ' id="active"';} ?>><a href="?p=news">News</a></li>
        <li<? if($_GET[p] == 'faq') { echo ' id="active"';} ?>><a href="?p=faq">FAQ</a></li>
        <li<? if($_GET[p] == 'about') { echo ' id="active"';} ?>><a href="?p=about">About</a></li>
        <li<? if($_GET[p] == 'contact') { echo ' id="active"';} ?>><a href="?p=contact">Contact</a></li>
        <li<? if($_GET[p] == 'stores') { echo ' id="active"';} ?>><a href="?p=stores">Stores</a></li>
      </ul>
    </div>
  </div>
</div>
...
?>
          ...
          <? if(!$_SESSION[access]){ echo '<li><a href="?p=register">Register</a></li>';} ?>
          <? if(isset($_SESSION[active]) && $_SESSION[active] == 0){ echo '<li><a href="?p=confirm">Confirm your Account</a></li>';} ?>
          <? if($_SESSION[access] == 100){ echo '<li><a href="?p=admin">Site Admin</a></li>';} ?>
          <? if($_SESSION[loggedin]){ echo '<li><a href="?p=logout">Log Out</a> </li>';}?>
</ul>
<? if($_GET[p]){ ?>
<!-- Sidebar Search Box -->
...
<? } ?>
        <p class="sidebarlt"><strong>Other Websites</strong><br />
              ...
              <a href="http://www.glbb.jp" target="_blank" alt="Internet Service Provider">Global BroadBand</a><br />
