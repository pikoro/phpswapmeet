...
class listings{
    ...
    function category_dropdown($catid=0){
    	...
    	foreach($array as $category){
				...
				if($category[parent] == 0){
				    if($catid == $category[id]){ $selected = ' selected ';} else { $selected = '';}
				...
				';
					...
					while($row=mysql_fetch_object($res)){
					    if($catid == $row->id){ $selected = ' selected ';} else { $selected = '';}
	...
	function show_item_listing($id){
		...
		if($array[custid]==$_SESSION[id]) echo '<span style="float:right; margin-top:-25px;"><a href="?p=edit_item&id='.$array[id].'">Edit</a></span>';
		echo '</div>';
		...
		if($array[custid]==$_SESSION[id]) echo '<span style="float:right; margin-top:-10px;"><a href="?p=delete_item&id='.$array[id].'">Delete</a></span>';
		echo '</div>';
		echo '</div>';
